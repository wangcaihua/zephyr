...
namespace {
string MetaToBytes(const Meta &meta, const Meta &shard_meta) {
  vector<string> lines;

  transform(meta.begin(), meta.end(), back_inserter(lines),
            [](const pair<string, string> &key_value) {
              return join_string({key_value.first, key_value.second}, ":");
            });
  transform(shard_meta.begin(), shard_meta.end(), back_inserter(lines),
            [](const pair<string, string> &key_value) {
              return join_string({"", key_value.first, key_value.second}, ":");
            });
...
string ShardToBytes(size_t shard_index, const Server &server) {
...
bool ZkServerRegister::RegisterShard(size_t shard_index, const Server &server,
  ...
  string shard_zk_child = ShardToBytes(shard_index, server);
  string shard_zk_path = join_string({zk_path_, shard_zk_child}, "/");
  string shard_meta_bytes = MetaToBytes(meta, shard_meta);
  ...
  if (state == ZOO_EXPIRED_SESSION_STATE) {
    ...
    auto *self = static_cast<ZkServerRegister *>(data);
    {
      ...
      while (self->zk_handle_ == nullptr) {
        ...
        self->zk_handle_ =
            zookeeper_init2(self->zk_addr_.c_str(), Watcher, 60000,
                nullptr, self, 0, ZkLogCallback);
      }
    }

    {
      lock_guard<mutex> lock(self->mu_);

      for (const auto &registered : self->registered_) {
        const string &shard_zk_path = registered.first;
        const string &shard_meta_bytes = registered.second;
